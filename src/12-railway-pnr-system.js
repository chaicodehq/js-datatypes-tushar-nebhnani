...
export function processRailwayPNR(pnrData) {
    if(typeof pnrData !== 'object' || !pnrData || typeof pnrData.pnr !== 'string' || pnrData.pnr.length !== 10 || !/^\d+$/.test(pnrData.pnr) || !pnrData.train || !Array.isArray(pnrData.passengers) || pnrData.passengers.length === 0) return null


    let rtn = {}
    const pnr = String(pnrData.pnr)
    const train = pnrData.train
    const passenger = pnrData.passengers
    const status = (code) => {
        switch (true) {
            case code.startsWith("B"):
            case code.startsWith("S"):
                return "CONFIRMED";
                    
            case code.startsWith("WL"):
                return "WAITING";
                    
            case code === "CAN":
                return "CANCELLED";
                    
            case code.startsWith("RAC"):
                return "RAC";
    ...
    const p = passenger.map((i) => {
        return {
            formattedName: `${i.name.padEnd(20)}(${i.age}/${i.gender})`,
            bookingStatus: i.booking,
            currentStatus: i.current,
            statusLabel: status(i.current),
            isConfirmed: status(i.current) === "CONFIRMED"
        }
    })

    const confirmedPassanger = passenger.filter((i) => status(i.current) === "CONFIRMED")
    const waitingPassanger = passenger.filter((i) => status(i.current) === "WAITING")
    const cancelledPassanger = passenger.filter((i) => status(i.current) === "CANCELLED")
    const racPassanger = passenger.filter((i) => status(i.current) === "RAC")
    

    rtn.pnrFormatted = `${pnr.slice(0, 3)}-${pnr.slice(3, 6)}-${pnr.slice(6,)}`;
    rtn.trainInfo = `Train: ${train.number} - ${train.name} | ${train.from} → ${train.to} | Class: ${pnrData.classBooked}`
    rtn.passengers = p
    rtn.summary = {
        totalPassengers: passenger.length,
        confirmed: confirmedPassanger.length,
        waiting: waitingPassanger.length,
        cancelled: cancelledPassanger.length,
        rac: racPassanger.length,
        allConfirmed: passenger.every(i => status(i.current) === "CONFIRMED"),
        anyWaiting: passenger.some(i => status(i.current) === "WAITING")
    }

    rtn.chartPrepared = passenger.every(p => {
        if (p.current === "CAN") return true; 
        return p.current.startsWith("B") || p.current.startsWith("S");
    });
